...
// this will bitch about ptr without this macro
...
mod tests {
    ...
    fn get_what_you_give() {
        fn property(k: u16, v: u16) -> TestResult {
            ...
            assert_eq!(None, sys_under_test.insert(k,v));
            assert_eq!(Some(&v), sys_under_test.get(&k));
    ...
    // Define the model so we can perform actions on this type
        ...
        T: Arbitrary,
    {
            ...
            G: Gen,
        {
            ...
            match i {
                // inclusive range operator; this is new, lots of old libs
                // use this operator
            ...
            for action in actions.into_iter() {
                match action {
                    Action::Insert(k, v) => {
                        ...
                        sys_under_test.insert(k,v));
                    }
                    Action::Lookup(k) => {
                        assert_eq!(model.get(&k), sys_under_test.get(&k));
                    }
                }
            }
            TestResult::passed()
